            ...
            if (newCourse.course === undefined) {
                ...
                newStudent.courses = newStudent.courses.map(c => {
                    if (c.course === classroom.course) {
                        return {
                            ...c,
                            classes: [...c.classes, classroom.name]
                        };
                    } else return c;
                });
            }
        ...
        if (
            ...
            original.courses = original.courses.filter(c => {
                if (c.course === classroom.course) {
                    return {
                        ...c,
                        classes: c.classes.filter(c => c !== classroom.name)
                    };
                } else return c;
            });
            await db
            ...
            return {
                ...
                u: users,
                error:
                    "Error: The student you are trying to remove or the classroom no longer exists."
            };
        }
    }
};
