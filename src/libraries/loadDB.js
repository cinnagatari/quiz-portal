...
const category = {
    ...
    add: async function addC(category) {
        ...
        let categories = [];
        if (versionState.c === "load" || JSON.parse(localStorage.getItem("categories")) === null) {
        ...
        if (!categories.includes(category)) {
            ...
            localStorage.setItem("categories", JSON.stringify([...categories, category].sort()));
            return { c: [...categories, category].sort() };
        } else {
            return {
                c: categories,
                error: "Error: The category you are trying to add has already been added."
            };
        }
    },
    ...
    edit: async function editC(newCategory, original) {
        ...
        let questions = [];
        if (versionState.q === "load" || JSON.parse(localStorage.getItem("questions")) === null) {
        ...
        if (!categories.includes(newCategory)) {
            ...
            return {
                q: questions,
                c: categories,
                error: "Error: The category you are trying to edit no longer exists or was changed to something else."
            };
        }
    },
    ...
    delete: async function deleteC(original, newCategory) {
        ...
        if (categories.includes(original)) {
            ...
            return {
                q: questions,
                c: categories,
                error: "Error: The category you are trying to delete no longer exists or was changed to something else."
            };
        }
    }
}
...
const question = {
    ...
    add: async function add(newQuestion) {
        ...
        let names = questions.map(q => q.name);
        if (!names.includes(newQuestion.name) && categories.includes(newQuestion.category)) {
            ...
            localStorage.setItem("categories", JSON.stringify(categories));
            return { q: [...questions, newQuestion], n: [...names, newQuestion.name], c: categories };
        } else {
            localStorage.setItem("questions", JSON.stringify(questions));
            localStorage.setItem("categories", JSON.stringify(categories));
            return {
                ...
                c: categories,
                error: "Error: The question name is already being used or the category no longer exists."
            };
        }
    },
    ...
    edit: async function editQ(newQuestion, original) {
        ...
        let names = questions.map(q => q.name);
        if (names.includes(original.name) && categories.includes(newQuestion.category)) {
            questions = questions.map(q => {
            ...
            if (newQuestion.name !== original.name)
                db.collection("questions").doc(original.name).delete();
            db.collection("questions").doc(newQuestion.name).set(newQuestion);
            localStorage.setItem("questions", JSON.stringify(questions));
            localStorage.setItem("categories", JSON.stringify(categories));
            ...
            return {
                ...
                c: categories,
                error: "Error: The new question name already exists or the new category no longer exists."
            };
        }
    },
